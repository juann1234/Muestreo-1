...
diseño <- svydesign(ids = ~diseno$upm_fic, strata = ~estred13, weights = ~pesotri, data = hogares_trim1)

# Estimar total de personas desocupadas
total_desocupados <- svytotal(~ht6, diseño, deff = TRUE)
...
SE_boot <- sd(ht6_boot$replicates)  # Bootstrap
...
# Estimar total de personas desocupadas
total_paysandu <- svytotal(~ht6, diseño_paysandu, deff = TRUE)
...
encuesta_HP_jovenes <- subset(encuesta_HP, e27 >= 18 & e27 <= 25) # Personas entre 18 y 25 años
...
svymean(~ht6, diseño_jovenes) # estimacion de tasa de desocupados en jovenes de entre 18 y 25 años.



confint(svytotal(~ht6, diseño_jovenes))
